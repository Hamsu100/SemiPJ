...
<section class="d-flex align-items-center dark-overlay bg-cover" style="background-image: url(http://tong.visitkorea.or.kr/cms/resource/81/2789081_image2_1.jpg);">
    <div class="container py-6 py-lg-7 text-white overlay-content">
        <div class="row">
            <div class="col-xl-8">
                <h1 class="display-3 fw-bold text-shadow">주변 시설 목록 찾아보기</h1>
                <p class="text-lg text-shadow mb-6">차 한 잔으로 몸도 마음도 쉬어가세요</p>
            </div>
        </div>
    </div>
</section>
...
<div class="container position-relative mt-n6 z-index-20 col-lg-12 mb-3">
    <ul class="nav nav-tabs search-bar-nav-tabs" role="tablist">
        ...
        <li class="nav-item me-2"><a class="nav-link active" href="<%=path%>/arround/list/cafe?lat=<%=bch_lat%>&lng=<%=bch_lng%>">카페/바</a></li>
                        ...
                        <div class="col-lg-12 py-5 p-xl-5">
                            <div class="row">
                                <div class="row col-lg-6">
                                    ...
                                    <% if (cafeList.isEmpty()) { %>
			                       	<div class="text-md-center"><br><br><br><br>가까운 거리에 카페/바가 없습니다</div>
			                        <% } %>
                                   	
                                    <!-- venue item-->
                                	<% for (Cafe cafeAll : cafeList) { %>
                                
                                    <div class="col-sm-6 mb-4 hover-animate">
                                        <div class="card h-auto border">
                                        
                                        	<!-- img -->
                                        	<% if (cafeAll.getCafe_img() == null || !cafeAll.getCafe_img().contains("http")) { %>
                                        	...
                                        	<% } else { %>
                                        		...
                                        		List<String> list = parse.textLineToList(cafeAll.getCafe_img());
                                        		%>
                                            ...
                                            <% } %>
                                                ...
                                                <a class="tile-link" href="<%=path%>/arround/view?type=4&lat=<%= cafeAll.getCafe_lat() %>&lng=<%= cafeAll.getCafe_long() %>&no=<%= cafeAll.getCafe_no() %>"></a>
                                                <div class="card-img-overlay-bottom z-index-20">
                                                    <h4 class="text-white text-shadow"><%= cafeAll.getCafe_name() %></h4>
                                                    <% nameList.add(cafeAll.getCafe_name()); %>
                                                </div>
                                            ...
                                            <div class="card-body">
                                            	<!-- distance -->
                                                <p class="text-sm text-muted mb-3 text-dark"><%= String.format("%.2f", cafeAll.getCafe_distance()) %> km</p>
                                                <% mapLevel = cafeAll.getCafe_distance(); %>
                                                
                                                <!-- content with substring -->
                                                <% if (cafeAll.getCafe_con() == null || cafeAll.getCafe_con().isEmpty()) { %>
                                                ...
                                                <% } else { %>
                                                	<% int lengthCut = 75; %>
                                                	<% if (cafeAll.getCafe_con().length() - 1 > lengthCut) { %>
                                               			<p class="text-sm text-muted mb-3"><%= cafeAll.getCafe_con().substring(0, lengthCut)+" ..." %></p>
                                                	<% } else { %>
                                               			<p class="text-sm text-muted mb-3"><%= cafeAll.getCafe_con() %></p>
                                                	<% } %>
                                                <% } %>
                                                
                                                <!-- tel -->
                                                <% if (cafeAll.getCafe_tel() == null || cafeAll.getCafe_tel().isEmpty()) { %>
                                                	<br>
                                                <% } else { %>
                                              		<p class="text-sm text-muted text-uppercase mb-1 text-dark"><%= cafeAll.getCafe_tel() %></p>
                                                <% } %>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- for js client -->
                                    <%
                                    latList.add(cafeAll.getCafe_lat());
                                    lngList.add(cafeAll.getCafe_long());
                                    %>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <nav aria-label=" Page navigation example">
                                            <ul class="pagination pagination-template d-flex justify-content-center">
                                                ...
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/cafe?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getPrvePage()%>"> <i class="fa fa-angle-left"></i></a>
                                                </li>
                                                ...
                                                <% for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) { %>
                                                	...
                                                	<% } else { %>
                                               			<li class="page-item"><a class="page-link" href="<%=path%>/arround/list/cafe?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=i%>"><%=i%></a></li>
                                                	<% } %>
                                                <% } %>	
                                                
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/cafe?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getNextPage()%>"> <i class="fa fa-angle-right"></i></a>
                                                </li>
                                            </ul>
                                        </nav>
                                    </div>
