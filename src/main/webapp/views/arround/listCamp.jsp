...
<%
	...
	List<Camp> campList = (List<Camp>) request.getAttribute("campList");
...
<section class="d-flex align-items-center dark-overlay bg-cover" style="background-image: url(https://gocamping.or.kr/upload/camp/1025/thumb/thumb_720_28962YGRML80hBAH1uQskMq7.jpg);">
    <div class="container py-6 py-lg-7 text-white overlay-content">
        <div class="row">
            <div class="col-xl-8">
                <h1 class="display-3 fw-bold text-shadow">주변 시설 목록 찾아보기</h1>
                <p class="text-lg text-shadow mb-6">주변에 좋은 캠핑장 있으면 소개시켜줘!</p>
            </div>
        </div>
    </div>
</section>
...
<div class="container position-relative mt-n6 z-index-20 col-lg-12 mb-3">
    <ul class="nav nav-tabs search-bar-nav-tabs" role="tablist">
        ...
        <li class="nav-item me-2"><a class="nav-link active" href="<%=path%>/arround/list/camp?lat=<%=bch_lat%>&lng=<%=bch_lng%>">캠핑장</a></li>
                        ...
                        <div class="col-lg-12 py-5 p-xl-5">
                            <div class="row">
                                <div class="row col-lg-6">
			                        ...
			                        <% if (campList.isEmpty()) { %>
			                       		<div class="text-md-center"><br><br><br><br>가까운 거리에 캠핑장이 없습니다</div>
			                        <% } %>
                                   	
                                    <!-- venue item-->
                                	<% for (Camp campAll : campList) { %>
                                    ...
                                    <div class="col-sm-6 mb-4 hover-animate">
                                        <div class="card h-auto border">
                                        	...
                                        	<% } else { %>
                                        		...
                                        		List<String> list = parse.textLineToList(campAll.getCamp_img());
                                        		%>
                                            ...
                                            <% } %>
                                                ...
                                                <a class="tile-link" href="<%=path%>/arround/view?type=2&lat=<%= campAll.getCamp_latitude() %>&lng=<%= campAll.getCamp_longitude() %>&no=<%= campAll.getCamp_no() %>"></a>
                                                <div class="card-img-overlay-bottom z-index-20">
                                                    <h4 class="text-white text-shadow"><%= campAll.getCamp_name() %></h4>
                                                    <% nameList.add(campAll.getCamp_name()); %>
                                                </div>
                                            ...
                                            <div class="card-body">
                                            	<!-- distance -->
                                                <p class="text-sm text-muted mb-3 text-dark"><%= String.format("%.2f", campAll.getCamp_distance()) %> km</p>
                                                <% mapLevel = campAll.getCamp_distance(); %>
                                                
                                                <!-- content with substring -->
                                                <% if (campAll.getCamp_content() == null || campAll.getCamp_content().isEmpty()) { %>
                                                ...
                                                <% } else { %>
                                                	<% int lengthCut = 75; %>
                                                	<% if (campAll.getCamp_content().length() - 1 > lengthCut) { %>
                                               			<p class="text-sm text-muted mb-3"><%= campAll.getCamp_content().substring(0, lengthCut)+" ..." %></p>
                                                	<% } else { %>
                                               			<p class="text-sm text-muted mb-3"><%= campAll.getCamp_content() %></p>
                                                	<% } %>
                                                <% } %>
                                                
                                                <!-- tel -->
                                                <% if (campAll.getCamp_tel() == null || campAll.getCamp_tel().isEmpty()) { %>
                                                	<br>
                                                <% } else { %>
                                              		<p class="text-sm text-muted text-uppercase mb-1 text-dark"><%= campAll.getCamp_tel() %></p>
                                                <% } %>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- for js client -->
                                    <%
                                    latList.add(campAll.getCamp_latitude());
                                    lngList.add(campAll.getCamp_longitude());
                                    %>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <nav aria-label=" Page navigation example">
                                            <ul class="pagination pagination-template d-flex justify-content-center">
                                                ...
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/camp?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getPrvePage()%>"> <i class="fa fa-angle-left"></i></a>
                                                </li>
                                                ...
                                                <% for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) { %>
                                                	...
                                                	<% } else { %>
                                               			<li class="page-item"><a class="page-link" href="<%=path%>/arround/list/camp?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=i%>"><%=i%></a></li>
                                                	<% } %>
                                                <% } %>	
                                                
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/camp?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getNextPage()%>"> <i class="fa fa-angle-right"></i></a>
                                                </li>
                                            </ul>
                                        </nav>
                                    </div>
