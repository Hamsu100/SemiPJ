...
<%
	...
	List<Res> resList = (List<Res>) request.getAttribute("resList");
...
<section class="d-flex align-items-center dark-overlay bg-cover" style="background-image: url(http://tong.visitkorea.or.kr/cms/resource/29/2654729_image2_1.jpg);">
    <div class="container py-6 py-lg-7 text-white overlay-content">
        <div class="row">
            <div class="col-xl-8">
                <h1 class="display-3 fw-bold text-shadow">주변 시설 목록 찾아보기</h1>
                <p class="text-lg text-shadow mb-6">배도 마음도 한가득 채워가세요</p>
            </div>
        </div>
    </div>
</section>
...
<div class="container position-relative mt-n6 z-index-20 col-lg-12 mb-3">
    <ul class="nav nav-tabs search-bar-nav-tabs" role="tablist">
        ...
        <li class="nav-item me-2"><a class="nav-link active" href="<%=path%>/arround/list/res?lat=<%=bch_lat%>&lng=<%=bch_lng%>">음식점</a></li>
                            ...
                            <div class="row">
                                <div class="row col-lg-6">
                                	...
                                	Collections.shuffle(contentNull);
                                   	...
                                   	<% if (resList.isEmpty()) { %>
			                       	<div class="text-md-center"><br><br><br><br>가까운 거리에 음식점이 없습니다</div>
			                        <% } %>
                                   	
                                    <!-- venue item-->
                                	<% for (Res resAll : resList) { %>
                                
                                    <div class="col-sm-6 mb-4 hover-animate">
                                        <div class="card h-auto border">
                                        
                                        	<!-- img -->
                                        	<% if (resAll.getRes_img() == null || !resAll.getRes_img().contains("http")) { %>
                                        	...
                                        	<% } else { %>
                                        		...
                                        		List<String> list = parse.textLineToList(resAll.getRes_img());
                                        		%>
                                            ...
                                            <% } %>
                                                ...
                                                <a class="tile-link" href="<%=path%>/arround/view?type=5&lat=<%= resAll.getRes_lat() %>&lng=<%= resAll.getRes_long() %>&no=<%= resAll.getRes_no() %>"></a>
                                                <div class="card-img-overlay-bottom z-index-20">
                                                    <h4 class="text-white text-shadow"><%= resAll.getRes_name() %></h4>
                                                    <% nameList.add(resAll.getRes_name()); %>
                                                </div>
                                            ...
                                            <div class="card-body">
                                            	<!-- distance -->
                                                <p class="text-sm text-muted mb-3 text-dark"><%= String.format("%.2f", resAll.getRes_distance()) %> km</p>
                                                <% mapLevel = resAll.getRes_distance(); %>
                                                
                                                <!-- content with substring -->
                                                <% if (resAll.getRes_con() == null || resAll.getRes_con().isEmpty()) { %>
                                                ...
                                                <% } else { %>
                                                	<% int lengthCut = 75; %>
                                                	<% if (resAll.getRes_con().length() - 1 > lengthCut) { %>
                                               			<p class="text-sm text-muted mb-3"><%= resAll.getRes_con().substring(0, lengthCut)+" ..." %></p>
                                                	<% } else { %>
                                               			<p class="text-sm text-muted mb-3"><%= resAll.getRes_con() %></p>
                                                	<% } %>
                                                <% } %>
                                                
                                                <!-- tel -->
                                                <% if (resAll.getRes_tel() == null || resAll.getRes_tel().isEmpty()) { %>
                                                	<br>
                                                <% } else { %>
                                              		<p class="text-sm text-muted text-uppercase mb-1 text-dark"><%= resAll.getRes_tel() %></p>
                                                <% } %>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- for js client -->
                                    <%
                                    latList.add(resAll.getRes_lat());
                                    lngList.add(resAll.getRes_long());
                                    %>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <nav aria-label=" Page navigation example">
                                            <ul class="pagination pagination-template d-flex justify-content-center">
                                                ...
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/res?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getPrvePage()%>"> <i class="fa fa-angle-left"></i></a>
                                                </li>
                                                ...
                                                <% for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) { %>
                                                	...
                                                	<% } else { %>
                                               			<li class="page-item"><a class="page-link" href="<%=path%>/arround/list/res?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=i%>"><%=i%></a></li>
                                                	<% } %>
                                                <% } %>	
                                                
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/res?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getNextPage()%>"> <i class="fa fa-angle-right"></i></a>
                                                </li>
                                            </ul>
                                        </nav>
                                    </div>
