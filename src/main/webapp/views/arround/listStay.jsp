...
<%
	...
	List<Stay> stayList = (List<Stay>) request.getAttribute("stayList");
    ...
    <div class="container py-6 py-lg-7 text-white overlay-content">
        <div class="row">
            <div class="col-xl-8">
                ...
                <p class="text-lg text-shadow mb-6">잠은 좋은곳에서 자야합니다! 꿀잠을 준비하세요</p>
            </div>
        </div>
    </div>
</section>
...
<div class="container position-relative mt-n6 z-index-20 col-lg-12 mb-3">
    <ul class="nav nav-tabs search-bar-nav-tabs" role="tablist">
  		<li class="nav-item me-2"><a class="nav-link active" href="<%=path%>/arround/list/stay?lat=<%=bch_lat%>&lng=<%=bch_lng%>">숙박</a></li>
                            ...
                            <div class="row">
                                <div class="row col-lg-6">
                                	...
                                	Collections.shuffle(contentNull);
                                   	...
                                   	<% if (stayList.isEmpty()) { %>
			                       	<div class="text-md-center"><br><br><br><br>가까운 거리에 숙박시설이 없습니다</div>
			                        <% } %>
                                   	
                                    <!-- venue item-->
                                	<% for (Stay stayAll : stayList) { %>
                                    ...
                                    <div class="col-sm-6 mb-4 hover-animate">
                                        <div class="card h-auto border">
                                        	...
                                        	<% } else { %>
                                        		...
                                        		List<String> list = parse.textLineToList(stayAll.getImg());
                                        		%>
                                            ...
                                            <% } %>
                                                ...
                                                <a class="tile-link" href="<%=path%>/arround/view?type=1&lat=<%= stayAll.getLatitude() %>&lng=<%= stayAll.getLongitude() %>&no=<%= stayAll.getStay_no() %>"></a>
                                                <div class="card-img-overlay-bottom z-index-20">
                                                    <h4 class="text-white text-shadow"><%= stayAll.getName() %></h4>
                                                    <% nameList.add(stayAll.getName()); %>
                                                </div>
                                            ...
                                            <div class="card-body">
                                            	<!-- distance -->
                                                <p class="text-sm text-muted mb-3 text-dark"><%= String.format("%.2f", stayAll.getDistance()) %> km</p>
                                                <% mapLevel = stayAll.getDistance(); %>
                                                
                                                <!-- content with substring -->
                                                <% if (stayAll.getContent() == null || stayAll.getContent().isEmpty()) { %>
                                                ...
                                                <% } else { %>
                                                	<% int lengthCut = 75; %>
                                                	<% if (stayAll.getContent().length() - 1 > lengthCut) { %>
                                               			<p class="text-sm text-muted mb-3"><%= stayAll.getContent().substring(0, lengthCut)+" ..." %></p>
                                                	<% } else { %>
                                               			<p class="text-sm text-muted mb-3"><%= stayAll.getContent() %></p>
                                                	<% } %>
                                                <% } %>
                                                
                                                <!-- tel -->
                                                <% if (stayAll.getTel() == null || stayAll.getTel().isEmpty()) { %>
                                                	<br>
                                                <% } else { %>
                                              		<p class="text-sm text-muted text-uppercase mb-1 text-dark"><%= stayAll.getTel() %></p>
                                                <% } %>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <!-- for js client -->
                                    <%
                                    latList.add(stayAll.getLatitude());
                                    lngList.add(stayAll.getLongitude());
                                    %>
                                    ...
                                    <div class="mt-3">
                                        ...
                                        <nav aria-label=" Page navigation example">
                                            <ul class="pagination pagination-template d-flex justify-content-center">
                                                ...
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/stay?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getPrvePage()%>"> <i class="fa fa-angle-left"></i></a>
                                                </li>
                                                ...
                                                <% for (int i = pageInfo.getStartPage(); i <= pageInfo.getEndPage(); i++) { %>
                                                	...
                                                	<% } else { %>
                                               			<li class="page-item"><a class="page-link" href="<%=path%>/arround/list/stay?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=i%>"><%=i%></a></li>
                                                	<% } %>
                                                <% } %>	
                                                
                                                <li class="page-item">
                                                    <a class="page-link" href="<%=path%>/arround/list/stay?lat=<%=bch_lat%>&lng=<%=bch_lng%>&page=<%=pageInfo.getNextPage()%>"> <i class="fa fa-angle-right"></i></a>
                                                </li>
                                            </ul>
                                        </nav>
                                    </div>
